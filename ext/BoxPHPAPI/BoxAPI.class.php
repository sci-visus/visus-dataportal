<?php 
  // This is a modified version of the BoxPHPAPI 1.1
  // hosted at https://github.com/misterfifi1/BoxPHPAPI
	...
	class Box_API {
		...
		public function get_user_by_login_box($login){
		...
		private function build_url($url, array $opts = array(), $api_func="") {
			if(!$url) $url = $this->api_url;
			$opts = $this->set_opts($opts);
			$base = "https://api.box.com/2.0/" . $url . $api_func . '?';
			$query_string = http_build_query($opts);
			$base = $base . $query_string;
			return $base;
		}
		...
		private function get($url, $output="") {
			$ch = curl_init();
